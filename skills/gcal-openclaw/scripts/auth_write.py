...
def main():
    ap = argparse.ArgumentParser(description="OAuth auth for Google Calendar (write events).")
    ap.add_argument("--account", required=True)
    ap.add_argument(
        "--config-dir",
        default=str(Path.home() / ".config" / "openclaw-gcal"),
    ...
    ap.add_argument("--port", type=int, default=18797, help="Local server port for OAuth redirect (tunnel flow)")
