# Azure AI Foundry (Managed Identity) → OpenClaw Compatibility Proxy

## Why we’re doing this
OpenClaw can talk to “model providers” that look like standard OpenAI/Anthropic APIs (usually with a static API key).
...
### 1) Use Kimi K2.5 as a heartbeat model
We can route Gateway heartbeats (periodic check-ins) to a cheaper model without changing the main chat/coding model.

- Primary model stays: `github-copilot/gpt-5.2`
- Heartbeat model becomes: `foundry/Kimi-K2.5`

### 2) Future: “Research mode” pipeline
The proxy is also the foundation for a planned research workflow:
...
- Minimizes attack surface by proxying only the required endpoints.
...
## Files
- `server.mjs` — Node.js proxy server
- `~/.config/systemd/user/openclaw-foundry-proxy.service` — systemd user service
...
The proxy normalizes this (copies `reasoning_content` into `content` when `content` is empty) so OpenClaw receives a standard content field.
